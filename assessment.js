'use strict';
  ...
  assessmentButton.onclick = () => {
    ...
    removeAllChildren (tweetDivided);//練習問題
  ...
  function assessment(userName) {
    // 文字列のコード番号を取得してそれを足し合わせる
    let sumOfCharCode = 0;
    for (let i = 0; i < userName.length; i++) {
      sumOfCharCode = sumOfCharCode + userName.charCodeAt(i);
    }
    //文字コード番号の合計を回答に数で割って添字の数値を求める
    const index = sumOfCharCode % answers.length;
    let rseult = answers[index];
    rseult = rseult.replace(/\{ userName \}/g,　userName);
    return rseult;
  }
...
console.assert(
  assessment('三郎') === assessment('三郎'),'入力が同じ名前なら、同じ診断結果を出力する」処理が正しくありません'
);
